...
const TranslatableCandidateWithoutEquality = ({ candidate, index, column, otherColumns, columns, candidates, allCandidates, onSelectDestinationColumn, disabled, showRank, t }) => {
  ...
  */
  ...
  const rankedSectionIsNotEmpty = columns && columns.hasOwnProperty("ranked") && columns["ranked"].hasOwnProperty("candidatesIds") && columns["ranked"]["candidatesIds"] && columns["ranked"]["candidatesIds"].length > 0;

  if (column && column.id === "not-ranked"){ // Current candidate currently appears in the "not-ranked" section
    if (!rankedSectionIsNotEmpty){
      otherPreferencesSelectOptions.push(SelectOption({value: "ranked", label: t('preferential_voting_ranked')}));
    }
  }
  else { // Current candidate currently appears in the "ranked" section
    otherPreferencesSelectOptions.push(SelectOption({value: "not-ranked", label: t("preferential_voting_not_ranked")}));
  }

  if (rankedSectionIsNotEmpty){
    const otherRankedCandidates = columns["ranked"]["candidatesIds"].map((candidateId, idx) => {return {"candidateId": candidateId, "index": idx};}).filter(cand => {return cand.candidateId != candidate.id && index+1 != cand.index;});
    let selectOptionsToAdd = otherRankedCandidates.map((cand) => {
      const candidate = allCandidates[cand.candidateId];
      const candidateLabel = t("preferential_voting_without_equality_move_candidate_above_position_x", {count: cand.index+1, candidate: candidate.content});
      const destinationColumnAndIndex = `ranked~${cand.index}`;
      return SelectOption({value: destinationColumnAndIndex, label: candidateLabel});
    });
    otherPreferencesSelectOptions = [...otherPreferencesSelectOptions, ...selectOptionsToAdd];
    
    // Add below bottom ranked candidate
    const bottomRankedCandidateId = columns["ranked"]["candidatesIds"][columns["ranked"]["candidatesIds"].length -1];
    const bottomRankedCandidate = allCandidates[bottomRankedCandidateId];
    if (bottomRankedCandidate.id != candidate.id){
      const belowLastLabel = t("preferential_voting_without_equality_move_candidate_below_position_x", {count: columns["ranked"]["candidatesIds"].length, candidate: bottomRankedCandidate.content});
      const BelowLastOption = SelectOption({value: "ranked", label: belowLastLabel});
      otherPreferencesSelectOptions = [...otherPreferencesSelectOptions, BelowLastOption];
    }
  }

  // Prepend initial option used as dropdown label
  const firstOption = SelectOption({value: "default", label: t("preferential_voting_move_candidate_to")});
...
const CandidateWithoutEquality = withTranslation()(TranslatableCandidateWithoutEquality);
...
const PreferentialVotingWithoutEqualityColumn = ({ column, label, otherColumns, columns, candidates, allCandidates, onSelectCandidateDestinationColumn, disabled
 }) => {
  const rendered_candidates = candidates.map((candidate, index) => {
    return e(
      CandidateWithoutEquality,
      {
        key: candidate.id,
        candidate,
        index,
        ...
        onSelectDestinationColumn: (event) => {
          ...
          const destinationColumnCandidateIndex = destination.length > 1 ? destination[1] : null;
          onSelectCandidateDestinationColumn(candidate.id, index, destinationColumn, destinationColumnCandidateIndex);
