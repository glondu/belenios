...
function TranslatablePreferentialVotingVoteRecap({
  question,
  question_index,
  uncryptedBallot,
  t,
  ...
  if (
    ...
    uncryptedBallot[question_index].forEach(
      function (candidate_selected_ranking, candidate_index) {
        ...
        if (
          !Array.isArray(
            candidatesIndexesGroupedByPreferenceLevel[
              selectedPreferenceLevelIndex
            ],
          )
        ) {
          candidatesIndexesGroupedByPreferenceLevel[
    ...
    renderedGradedCandidates = candidatesIndexesGroupedByPreferenceLevel.map(
      (preference_level_candidates, preference_level_index) => {
        return e(PreferentialVotingVoteRecapForPreferenceLevel, {
          preference_level_title: buildColumnLabel(
            null,
            preference_level_index,
            t,
          ),
          preference_level_candidates: preference_level_candidates.map(
    ...
    if (notRankedCandidatesIndexes.length > 0) {
      renderedGradedCandidates.push(
        e(PreferentialVotingVoteRecapForPreferenceLevel, {
...
const PreferentialVotingVoteRecap = withTranslation()(
  TranslatablePreferentialVotingVoteRecap,
);
