...
function PreferentialVotingWithoutEqualityVoteRecapForPreferenceLevel({ preference_level_title, preference_level_candidates }){
...
function TranslatablePreferentialVotingWithoutEqualityVoteRecap({ question, question_index, uncryptedBallot, t }){
  ...
  , 0) === 0){
    ...
    uncryptedBallot[question_index].forEach(function(candidate_selected_ranking, candidate_index){
      if (candidate_selected_ranking === 0){
        ...
        rankedCandidates[selectedPreferenceLevelIndex] = `${selectedPreferenceLevelIndex+1}. ${questionCandidates[candidate_index]}`;
      }
    });
    ...
    if (rankedCandidates.length > 0){
      renderedGradedCandidates.push(
        e(
          PreferentialVotingWithoutEqualityVoteRecapForPreferenceLevel,
          {
            preference_level_title: notRankedCandidatesIndexes.length == 0 ? "" : t("preferential_voting_ranked"),
...
const PreferentialVotingWithoutEqualityVoteRecap = withTranslation()(TranslatablePreferentialVotingWithoutEqualityVoteRecap);

export { TranslatablePreferentialVotingWithoutEqualityVoteRecap, PreferentialVotingWithoutEqualityVoteRecap };
