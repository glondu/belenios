...
class PreferentialVotingWithoutEqualityApp extends React.Component {
  ...
  buildUserVoteForAllCandidatesInQuestion() {
    ...
    this.state.columnOrder.forEach((currentColumnId, currentColumnIndex) => {
      this.state.columns[currentColumnId].candidatesIds.forEach((currentCandidateId, currentCandidateIndex) => {
        let candidate = this.state.candidates[currentCandidateId];
        if (!candidate){
          alert("could not find candidate");
        }
        else {
          updatedVoteForCandidates[candidate.initialIndex] = (currentColumnId === 'not-ranked') ? undefined : currentCandidateIndex;
        }
      });
    });
  ...
  render() {
    ...
    const children = this.state.columnOrder.map(
      (columnId, index) => {
        ...
        return e(
          PreferentialVotingWithoutEqualityColumn,
          {
            key: column.id,
            column: column,
            ...
            onSelectCandidateDestinationColumn: (candidateId, sourceColumnCandidateIndex, destinationColumnId, destinationColumnCandidateIndex) => {
              if(destinationColumnCandidateIndex === undefined || destinationColumnCandidateIndex === null){
                destinationColumnCandidateIndex = this.state.columns[destinationColumnId].candidatesIds.length;
              }
              this.moveCandidate(candidateId, column.id, destinationColumnId, sourceColumnCandidateIndex, destinationColumnCandidateIndex);
            },
...
function TranslatablePreferentialVotingWithoutEqualityBigCandidatesList({ identifierPrefix, candidates, blankVoteIsAllowed, renderedBlankVoteComponent, currentUserVoteForQuestion, currentCandidatesHavingAlertsForQuestion, dispatchUpdateUserVoteForQuestion, t }){
  /*
  TODO:
  ...
  */
  ...
  return e(
    ...
    e(
      PreferentialVotingWithoutEqualityApp,
      {
        initialData,
        dispatchUpdateUserVoteForQuestion,
...
function TranslatablePreferentialVotingWithoutEqualityCandidatesList({ identifierPrefix, candidates, blankVoteIsAllowed, currentUserVoteForQuestion, currentCandidatesHavingAlertsForQuestion, dispatchUpdateUserVoteForQuestion, t }){
  ...
  return e(
    ...
    e(
      TranslatablePreferentialVotingWithoutEqualityBigCandidatesList,
      {
        identifierPrefix,
        candidates,
        blankVoteIsAllowed,
...
const PreferentialVotingWithoutEqualityCandidatesList = withTranslation()(TranslatablePreferentialVotingWithoutEqualityCandidatesList);

export { PreferentialVotingWithoutEqualityCandidatesList, TranslatablePreferentialVotingWithoutEqualityCandidatesList, buildColumnLabel };
