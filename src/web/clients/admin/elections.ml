      ...
      let* status =
        if is_draft then
          if curr_tab = x then Lwt.return `Doing
            ...
            Lwt.return (if SMap.is_empty voter_list then `Todo else `Done)
        else Lwt.return `DDone
      in
      {
        title = s_ "Voter list";
...
let voters_content () =
  ...
  let with_address, with_weight =
    ...
    try
      voters |> SMap.to_seq
      |> Seq.fold_left
           (fun (with_address, with_weight) (_, (v : voter)) ->
             ...
             if show_only_missing && voted then accu
               ...
               List.flatten
                 [
                   ...
                   (if with_weight then
                      [ td [ txt @@ Weight.to_string weight ] ]
                    else []);
                   (if is_draft then []
                    else [ td [ txt (if voted then "X" else "—") ] ]);
                   [ td ~a:[ a_class [ "clickable" ] ] (erv v.login ()) ];
                 ]
