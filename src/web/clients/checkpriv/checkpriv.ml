...
let do_election uuid election get_private_key =
    ...
    try
      ...
      | `String x ->
         ...
         if W.G.compare public_key x.trustee_public_key = 0 then
           Some (Option.value ~default:(s_ "anonymous trustee") x.trustee_name)
         else None
      | _ -> raise Exit
    with
    | _ -> fun _ -> None
  ...
  List.find_map
    (function
     | `Single x -> find_single x
     | `Pedersen x -> find_pedersen x
    ) trustees
  |> show_result
     ...
     let name =
       ...
       List.find_map
         (fun x ->
           ...
           if G.(compare (g **~ private_key) y) = 0 then
             Some x.trustee_name
           else None
         ) trustees
     in
     ...
     let name =
       List.find_map
         (fun x ->
           ...
           if G.compare vk y.cert_verification = 0 then
             Some x.trustee_name
           else None
         ) trustees
     in
  ...
  let elt =
    div
      [
        div
          [
            txt @@ s_ "Please enter your private key:";
            txt " ";
            raw_elt;
          ];
        div
          [
            txt @@ s_ "Or load it from a file:";
            txt " ";
            file_elt;
          ];
      ]
...
let onload () =
  ...
  let@ configuration =
    perform (s_ "server configuration") configuration_of_string "../api/configuration"
  in
  let module UiBase =
    struct
  ...
  let button =
    ...
    | _ ->
       ...
       | None ->
          let@ draft =
            try_perform (s_ "draft election parameters") draft_of_string
              (Printf.sprintf "../api/drafts/%s" uuid)
          in
          match draft with
          | None ->
             Printf.ksprintf alert (f_ "There is no election with UUID %s on this server!") uuid;
             Lwt.return_unit
