...
follows:
\begin{enumerate}
\item compute $s=\secret(c)$ and $S=g^s$
\item pick a random $w\in\Z_q$
\item compute $A=g^w$
\item compute $\textsf{proof}$ as follows:
  \begin{enumerate}
  \item $\challenge=\Hash_\siglabel(S,A,\gamma_1,\dots,\gamma_l)\mod q$
...
Signatures are verified as follows ($S$ can be obtained from the
surrounding ballot):
\begin{enumerate}
\item compute $A=g^\response\times S^\challenge$
\item check that $\challenge=\Hash_\siglabel(S,A,\gamma_1,\dots,\gamma_l)\mod q$
\end{enumerate}
