...
\item for $z\in[1\dots \mu]$,
  ...
  \item $\mathcal{T}_z$ generates a \hyperref[polynomials]{$\poly$} $P_z$ and
    ...
    \begin{itemize}
    \item the coefficients of $f_z$, as well as  $\Gamma$,
      self signed and encrypted, in order to store them for the next
      step
    \item the signed exponentiated coefficients of $f_z$,
      to be sent to all trustees
...
generated as described in section~\ref{pki}. Each certificate comes
with a \texttt{context} containing the string description of the
group, the names of the trustees participating in the Pedersen
protocol, the threshold and the index (starting at 1) of the owner of
the certificate.
\begin{gather*}
...
\begin{gather*}
  \texttt{polynomial}=\left\{
    \begin{array}{rcl}
      \textsf{polynomial}&:&\texttt{raw\_polynomial}\ \texttt{sent\_msg}\\
...
\item the \textsf{polynomial} field is:
  \[\textsf{send}(\texttt{\dstprefix-polynomial},\textsf{sk}_i,\textsf{ek}_i,M)\]
  where $M$ is a \texttt{raw\_polynomial} structure:
  \begin{gather*}
...
\item the \textsf{secrets} field is $(s_1,\dotsc,s_m)$ where:
  ...
  \end{gather*}
  filled with $s_{ij}$
\item the \textsf{coefexps} field is a signed message using domain
  separation tag: \[\texttt{\dstprefix-pedersen-coefexps}\] containing
  a \texttt{raw\_coefexps} structure:
  \begin{gather*}
    \texttt{coefexps}=\texttt{raw\_coefexps}\ \texttt{signed\_msg}
    \qquad
...
\item the \textsf{signature} field is computed as follows:
  ...
  \item let $M$ be the following structure:
    ...
    and \textsf{coefexps} is set to $A_{i0},\dotsc,A_{it}$
  \item
