...
generated as described in section~\ref{pki} and the hash of the
$\texttt{coefexps}$ structure computed in
section~\ref{derived-polynomials}. Each certificate comes with a
\texttt{context} containing the string description of the group, the
names of the trustees participating in the Pedersen protocol, the
threshold and the index (starting at 1) of the owner of the
certificate.
\begin{gather*}
  ...
  \alpha\ \texttt{cert\_keys}=\left\{
    ...
    \end{array}
  \right\}\\
  \texttt{cert}'=\texttt{full\_context}\ \texttt{cert\_keys}\ \texttt{signed\_msg}\\
  \texttt{cert}=\I\ \texttt{cert\_keys}\ \texttt{signed\_msg}
\end{gather*}
A $\texttt{cert}'$ message is signed with the signing key associated
to \textsf{verification} and domain separation tag
\texttt{\dstprefix-pedersen-cert}. In contexts where
$\texttt{common\_context}$ is known, a $\texttt{cert}$' is stored in a
compressed form $\texttt{cert}$ containing only the index (but the
signature is still over the full context).
