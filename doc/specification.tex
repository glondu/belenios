...
\item $\mathcal{C}$ generates random \hyperref[credentials]{private
    credentials and salts} $c_1,\dots,c_n$ and $s_1,\dots,s_n$ and
  computes
  \[L=\textsf{sort}((\public(c_1,s_1),w_1,\mathcal{V}_1,s_1),\dots,(\public(c_n,s_n),w_n,\mathcal{V}_n,s_n))\]
...
\item \label{item-forget} (optional) $\mathcal{C}$ forgets $c_1,\dots,c_n$
 ...
 \item $\mathcal{S}$ checks that the $\mathcal{V}_i$ and the $w_i$ in
   $L$ are correct, and that all public credentials and salts are
   distinct, and extract the salt list as $\texttt{salts}$;
...
\item $\mathcal{S}$ publishes $\texttt{salts}$
...
\item $\mathcal{V}$ uses the index in her secret credential $c$ to get
  her public credential $\hat c$ (from election public data) and her
  salt $s$ (from \texttt{salts}), and checks that
  $\hat c=\public(c,s)$
...
$\texttt{XXX-XXXX-XXX-XXXX-NNNN}$, where the 14 $\texttt{X}$
characters are taken from the Base58 alphabet:
\[\texttt{123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz}\]
The string $\texttt{XXXXXXXXXXXXXX}$ will be called the \emph{prefix}
of $c$. $\texttt{NNNN}$ is a base 10 number that serves as an index to
locate the salt associated to $c$ in the list of salts and the
matching public key in the election public data. A \emph{salt} $s$ is
a string of 22 characters from the Base58 alphabet.

From $c$ and $s$, a secret exponent $x=\secret(c,s)$ is derived by
using PBKDF2 (RFC 2898) with:
\begin{itemize}
\item the prefix of $c$ as password;
\item HMAC-SHA256 (RFC 2104, FIPS PUB 180-2) as pseudorandom function;
\item the concatenation of the $\uuid$ of the election and $s$ as salt;
\item $100\,000$ iterations
\end{itemize}
and an output size of 2 blocks, which is interpreted as a big-endian
512-bit number and then reduced modulo $q$ to form $x$.  From this
secret exponent, a public key $\public(c,s)=g^x$ is computed.
...
credential $S=g^{\secret(c,s)}$ ($c$ being the secret credential, $s$
being the associated salt) of the voter. The hash (or
