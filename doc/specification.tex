...
\newcommand{\dstprefix}{BELENIOS-V2}
...
The variant considered in Belenios is proved
in~\cite{Pereira-proofDKG-eprint2024}.
%described in~\cite{wpes2013} and
...
\item $\mathcal{C}$ generates random \hyperref[credentials]{private
    credentials} $c_1,\dots,c_n$ and computes
  \[L=\textsf{sort}((\public(c_1),w_1,\mathcal{V}_1),\dots,(\public(c_n),w_n,\mathcal{V}_n))\]
...
\item \label{item-forget} (optional) $\mathcal{C}$ forgets $c_1,\dots,c_n$
 ...
 \item $\mathcal{S}$ checks that the $\mathcal{V}_i$ and the $w_i$ in
   $L$ are correct, and that all public credentials are
   distinct;
...
\item $\mathcal{C}$ checks that the list of public credentials in $L$
  is exactly the one that appears on the public data of the election.
\end{enumerate}
...
private credentials (but disables credential recovery, see
...
The original Pedersen DKG scheme~\cite{Pedersen} assumes a secret and authenticated
channel between each pair of trustees. Since our trustees do not even
have a PKI, we first require that each trustee generates a fresh key
pair, that they then use to encrypt and sign messages for the other
trustees. All messages are sent and received (encrypted) through the voting server,
in order to simplify the communication infrastructure.
\begin{enumerate}
...
\item for $z\in[1\dots \mu]$,
  ...
  \item $\mathcal{T}_z$ generates a \hyperref[polynomials]{$\poly$} $P_z$ and
    sends it to $\mathcal{S}$. This corresponds to the actual first
    step of the Pedersen protocol. Each trustee generates a secret
    polynomial $f_z$. $P_z$ consists in particular of:
    \begin{itemize}
    \item a signature of $\Gamma$
    \item the exponentiated coefficients of $f_z$, to be sent to all
      trustees
    \item the evaluation of the polynomial $s_{z,z'}=f_z(z')$, encrypted for
      trustee $\mathcal{T}_{z'}$ and signed by $\mathcal{T}_z$, for all $z'$
    \end{itemize}
  \item $\mathcal{S}$ checks $P_z$
  \end{enumerate}
\item for $z\in[1\dots \mu]$, $\mathcal{S}$ computes a
  \hyperref[vinputs]{$\vinput$} $\textsf{vi}_z$, that corresponds to
  the aggregation of the messages encrypted for $z$, collected from
  all the trustees
\item for $z\in[1\dots \mu]$,
  \begin{enumerate}
  \item $\mathcal{S}$ sends $\Gamma$ to $\mathcal{T}_z$ and $\mathcal{T}_z$ checks it
  ...
  \item $\mathcal{T}_z$ computes a \hyperref[voutputs]{$\voutput$} $\textsf{vo}_z$ and
    sends it to $\mathcal{S}$. It corresponds to the resulting (signed) public
    key share of $\mathcal{T}_z$, with a proof of knowledge of the associated secret key.
...
\begin{enumerate}
\item $\mathcal{V}$ gets public data of $E$
\item $\mathcal{V}$ uses the index in her secret credential $c$ to get
  her public credential $\hat c$ (from election public data), and checks that
  $\hat c=\public(c)$
...
\subsection{Credential recovery}\label{sec:recovery}
...
\item she computes $\hat{B}=\textsf{last}(B)$, the list of ballots
  ...
  \item she checks that $D$ is correctly chained, that is, each event correctly
    ...
    \item she checks that her view is consistent with the fingerprints
      displayed on the election main page.
  \end{itemize}
...
instead the command line tools \texttt{election verify} and
\texttt{election verify-diff} can be used.

\subsubsection{During and after the tally}
...
any), the list $\Delta$ of \hyperref[partial-decryptions]{partial
  ...
  \item she checks that $B$ corresponds to the board
    ...
    \item she computes $\hat{B}=\textsf{last}(B)$, that is, she keeps
      ...
      \item she checks that the   \hyperref[tally]{$\etally$}
        corresponds to $\hat{B}$;
 \item as soon as they are available, she checks that the proofs in $\Sigma$ and $\Delta$, and the result $r$, are valid
   w.r.t. $\hat{B}$;
      \item she checks that her view is consistent with the fingerprints
      displayed on the election main page.
\end{enumerate}
...
There is no tool support on the web interface for these checks,
instead the command line tool \texttt{election verify} can be used.
...
During an election, all public data is published in a dynamic
...
\subsubsection{Deriving keys}

The (private) signing key $\textsf{sk}$ and decryption key
$\textsf{dk}$ are derived as follows:
\begin{itemize}
\item $(\textsf{sk})\leftarrow\textsf{HashToField}(\texttt{\dstprefix-derive\_sk},s,1)$
\item $(\textsf{dk})\leftarrow\textsf{HashToField}(\texttt{\dstprefix-derive\_dk},s,1)$
\end{itemize}
The corresponding (public) verification key $\textsf{vk}$ and
encryption key $\textsf{ek}$ are derived as follows:
\begin{itemize}
\item $\textsf{vk}\leftarrow g^{\textsf{sk}}$
\item $\textsf{ek}\leftarrow g^{\textsf{dk}}$
\end{itemize}

\subsubsection{Signing}
\label{pki-signing}

Signing --- denoted as $\textsf{sign}(d,\textsf{sk},M)$ --- takes a
domain separation tag $d$, a signing key $\textsf{sk}$ and a
\textsf{message} $M$ (of type $\alpha$ serializable as a $\jstring$),
computes a \textsf{signature} and produces a
$\alpha\ \texttt{signed\_msg}$. We use a (Schnorr-like)
non-interactive zero-knowledge proof.

\begin{gather*}
  \alpha\ \texttt{signed\_msg}=\left\{
    \begin{array}{rcl}
      \textsf{message}&:&\alpha\\
  ...
  \right\}
\end{gather*}
Let $m$ be the concatenations of the string serialization of
$\textsf{vk}$, a verbatim \verb=|=, and the string serialization of
$M$.  To compute the \textsf{signature},
\begin{enumerate}
\item pick a random $w\in\Z_q$
\item compute the commitment $A=g^w$
\item compute the \textsf{challenge} as $\textsf{HashGroup}(d,m,(A))$
...
\item check that $\textsf{challenge}=\textsf{HashGroup}(d,m,(A))$
\end{enumerate}

\subsubsection{Encrypting}

Encrypting --- denoted as $\textsf{encrypt}(d,\textsf{ek},M)$ ---
takes a domain separation tag $d$, an encryption key $\textsf{ek}$ and
a message $M$ (of type $\alpha$ serializable as a $\jstring$), and
produces an $\alpha\ \texttt{encrypted\_msg}$. We use an El Gamal-like
system.

\begin{gather*}
  \alpha\ \texttt{encrypted\_msg}=\left\{
    \begin{array}{rcl}
      \textsf{algorithm}&:&\jstring\\
      \textsf{alpha}&:&\G\\
      \textsf{beta}&:&\G\\
      \textsf{data}&:&\jstring
    \end{array}
...
\item compute $\textsf{data}$ as the hexadecimal encoding of the
  (symmetric) encryption of $M$ using $\textsf{algorithm}$
  (e.g. \texttt{AES-GCM}) with
  $\textsf{SHA256}(d\texttt{-key|}g^s)$ as the key and
  $\textsf{SHA256}(d\texttt{-iv|}g^r)$ as the initialization vector
\end{enumerate}
To decrypt an \texttt{encrypted\_msg} using a decryption key \textsf{dk}:
\begin{enumerate}
\item compute the symmetric key as $\textsf{SHA256}(d\texttt{-key|}\textsf{beta}/(\textsf{alpha}^{\textsf{dk}}))$
\item compute the initialization vector as $\textsf{SHA256}(d\texttt{-iv|}\textsf{alpha})$
...
generated as described previously and a domain-specific context.
\begin{gather*}
  \alpha\ \texttt{cert\_keys}=\left\{
    \begin{array}{rcl}
      \textsf{context}&:&\alpha\\
...
\begin{gather*}
  \pk=\G\qquad\sk=\Z_q\\
  \texttt{raw\_trustee\_public\_key}=\left\{
    \begin{array}{rcl}
      \pklabel&:&\pk\\
      \textsf{name}&:&\jstring
    \end{array}
  \right\}\\
  \tpk=\texttt{raw\_trustee\_public\_key}\ \texttt{signed\_msg}
\end{gather*}
...
threshold mode.  The corresponding $\pklabel$ is $X=g^x$. A
$\texttt{raw\_trustee\_public\_key}$ is a bundle of this public key
with a $\textsf{name}$. Given such a
$\texttt{raw\_trustee\_public\_key}$ $S$, a $\tpk$ is computed with:
\[\textsf{sign}(\texttt{\dstprefix-verification\_key},X,S)\]

\subsection{Basic trustees}
\label{basic-trustees}

The most basic kind of trustee derives all their secrets from a secret
seed $s$. The (private) partial decryption key is derived from the
secret seed $s$ using (see~\ref{hash-group}):
\[\textsf{HashGroup}(\texttt{\dstprefix-basic\_key},s,())\]

They create a certificate as described in section~\ref{certificates},
signed with domain separation tag $\texttt{\dstprefix-basic-cert}$,
and use its signing key to sign the partial decryption verification
key with domain separation tag
$\texttt{\dstprefix-outer-verification\_key}$.

\begin{gather*}
  \texttt{basic\_parameters}=\left\{
    \begin{array}{rcl}
      \textsf{cert}&:&\texttt{null}\ \texttt{cert\_keys}\ \texttt{signed\_msg}\\
      \textsf{verification\_key}&:&\texttt{trustee\_public\_key}\ \texttt{signed\ msg}
    \end{array}
  \right\}\\
\end{gather*}

\subsection{Threshold trustees}
\label{threshold}

\subsubsection{Derived polynomials}
\label{derived-polynomials}

Each trustee $\mathcal{T}_i$ derives from the same seed $s$ as in
section~\ref{pki} a polynomial:
\[
  f_i(x)=a_{i0}+a_{i1}x+\dotsb+a_{it}x^t\in\Z_q[x]
\]
where the coefficients are generated with:
\[
  \textsf{HashToField}(\texttt{\dstprefix-pedersen-polynomial},s,t+1)
\]
$\mathcal{T}_i$ then computes exponentiated coefficients
$A_{i0},\dots,A_{it}$:
\[
  A_{ij}=g^{a_{ij}}
\]
and fills in a $\texttt{coefexps}$ structure with those
exponentiations:
\begin{gather*}
  \texttt{coefexps}=\left\{
    \begin{array}{rcl}
...
\subsubsection{Pedersen certificates}

A Pedersen certificate is a \texttt{signed\_msg} encapsulating a
serialized \texttt{cert\_keys} structure as described in
section~\ref{certificates} with \textsf{context} set to a
\texttt{pedersen\_context}.
\begin{gather*}
  \alpha\ \texttt{pedersen\_context} =\left\{
    \begin{array}{rcl}
      \textsf{context}&:&\alpha\\
      \textsf{coefexps}&:&\hash
    \end{array}
  \right\}
\end{gather*}
\textsf{coefexps} is the hash of the $\texttt{coefexps}$ structure
computed in section~\ref{derived-polynomials}. Each
\texttt{pedersen\_context} comes with a nested \textsf{context}
containing the string description of the group, the names of the
trustees participating in the Pedersen protocol, the threshold and the
index (starting at 1) of the owner of the certificate.
\begin{gather*}
  \texttt{common\_context}=\left\{
    \begin{array}{rcl}
      \textsf{group}&:&\jstring\\
      \textsf{names}&:&\jstring^\ast\\
      \textsf{threshold}&:&\I
    \end{array}
  \right\}\\
  \texttt{full\_context}=\left\{
    \begin{array}{rcl}
      \textsf{context}&:&\texttt{common\_context}\\
      \textsf{index}&:&\I
    \end{array}
  \right\}\\
  \texttt{pedersen\_cert}'=\texttt{full\_context}\ \texttt{pedersen\_context}\ \texttt{cert\_keys}\ \texttt{signed\_msg}\\
  \texttt{pedersen\_cert}=\I\ \texttt{pedersen\_context}\ \texttt{cert\_keys}\ \texttt{signed\_msg}
\end{gather*}
A $\texttt{pedersen\_cert}'$ message is signed with the signing key
associated to \textsf{verification} and domain separation tag
\texttt{\dstprefix-pedersen-cert}. In contexts where
$\texttt{common\_context}$ is known, a $\texttt{pedersen\_cert}'$ is
stored in a compressed form $\texttt{pedersen\_cert}$ containing only
the index (but the signature is still over the full context).

\subsubsection{Channels}
\label{channels}

\begin{gather*}
  \alpha\ \texttt{channel\_msg}=\left\{
    \begin{array}{rcl}
      \textsf{recipient}&:&\G\\
      \textsf{message}&:&\alpha
    \end{array}
  \right\}\\
  \alpha\ \texttt{sent\_msg}=
  \alpha\ \texttt{channel\_msg}\ \texttt{signed\_msg}\ \texttt{encrypted\_msg}
\end{gather*}

Given a domain separation tag $d$, a \textsf{message} is sent securely
from \textsf{sk} (a signing key) to \textsf{recipient} (an encryption
key) by encapsulating it in a \texttt{channel\_msg}, signing it with
\textsf{sk} and domain separation tag $d\texttt{-channel\_msg\_inner}$
and finally encrypting it with \textsf{recipient} and domain
separation tag $d\texttt{-channel\_msg\_outer}$ . The resulting
$\alpha\ \texttt{sent\_msg}$ will be denoted by:
\[\textsf{send}(d,\textsf{sk},\textsf{recipient},\textsf{message})\]
and can be transmitted using a third-party (typically the election
server).
...
\begin{gather*}
  \texttt{polynomial}=\left\{
    \begin{array}{rcl}
      \textsf{secrets}&:&\texttt{secret}\ \texttt{sent\_msg}^\ast\\
      \textsf{coefexps}&:&\texttt{coefexps}\\
      \textsf{signature}&:&\proof
    \end{array}
  \right\}
\end{gather*}
Suppose $\mathcal{T}_i$ is the trustee who is computing.
$\mathcal{T}_i$ computes $s_{ij}=f_i(j)\mod q$ for $j=1,\dotsc,m$,
where $f_i$ is the polynomial derived in
section~\ref{derived-polynomials}. $\mathcal{T}_i$ then fills the
\texttt{polynomial} structure as follows:
\begin{itemize}
\item the \textsf{secrets} field is $(s_1,\dotsc,s_m)$ where:
  \[ s_j =
    \textsf{send}(\texttt{\dstprefix-secret},\textsf{sk}_i,\textsf{ek}_j,M_{ij}) \]
  and $M_{ij}$ is a \texttt{secret} structure:
  \begin{gather*}
    \texttt{secret}=\left\{
      \begin{array}{rcl}
        \textsf{secret}&:&\Z_q
  ...
  \end{gather*}
  filled with $s_{ij}$;
\item the \textsf{signature} field is computed as follows:
  \begin{enumerate}
  \item let $M$ be the following structure:
    \begin{gather*}
      \texttt{certs}=\left\{
        \begin{array}{rcl}
          \textsf{context}&:&\texttt{common\_context}\\
      ...
      \right\}
    \end{gather*}
    where \textsf{certs} is set to $\Gamma=\gamma_1,\dotsc,\gamma_m$
  \item
    $M'\leftarrow\textsf{sign}(\texttt{\dstprefix-pedersen-certs},\textsf{sk}_i,M)$
  \item the field is set to the \textsf{signature} field of $M'$
  \end{enumerate}
\item the \textsf{coefexps} field is the \texttt{coefexps} structure
  from section~\ref{derived-polynomials}.
\end{itemize}
...
\item the \textsf{signatures} field is
  $\textsf{signature}(P_1),\dotsc,\textsf{signature}(P_m)$
\end{itemize}
Note that \textsf{coefexps} and \textsf{signatures} are the same for
all trustees.
...
$\textsf{vo}_j$.
\begin{gather*}
  \texttt{partial\_decryption\_key}=\left\{
    \begin{array}{rcl}
      \textsf{decryption\_key}&:&\Z_q
    \end{array}
  \right\}\\
  \texttt{threshold\_verification\_key}=\texttt{trustee\_public\_key}\ \texttt{signed\_msg}\\
  \texttt{voutput}=\left\{
    \begin{array}{rcl}
      \textsf{private\_key}&:&\texttt{partial\_decryption\_key}\ \texttt{sent\_msg}\\
      \textsf{public\_key}&:&\texttt{threshold\_verification\_key}
    \end{array}
  \right\}
\end{gather*}
...
\item \textsf{private\_key} is set to:
  \[\textsf{send}(\texttt{\dstprefix-pedersen-decryption\_key},\textsf{sk}_j,\textsf{ek}_j,S_j)\]
  where $S_j$ is filled with $\mathcal{T}_j$'s (private) decryption
  key:
  \[
    S_j=\sum_{i=1}^m s_{ij}\mod q
  \]
\item \textsf{public\_key} embeds a
  \hyperref[trustee-keys]{\texttt{trustee\_public\_key}} structure
  built using $S_j$ as private key, signed with the certificate key of
  $\mathcal{T}_j$ and the following domain separation tag:
  \[\texttt{\dstprefix-outer-verification\_key}\]
  Note that the result has two layers of signatures: an inner one,
  which acts as a proof of knowledge of the partial decryption key,
  and an outer one, which acts as a certification of the verification
  key using the threshold PKI.
\end{itemize}
The administrator checks $\textsf{vo}_j$ as follows:
\begin{itemize}
\item check that:
  \[
    \textsf{message}(\textsf{message}(\textsf{public\_key}(\textsf{vo}_j)))=\prod_{i=1}^m \prod_{k=0}^t (A_{ik})^{j^k}
  \]
\item check all signatures in $\textsf{vo}_j$
\end{itemize}
...
\begin{gather*}
  \texttt{threshold\_parameters}=\left\{
    \begin{array}{rcl}
      ...
      \textsf{coefexps}&:&\texttt{coefexps}^\ast\\
      \textsf{signatures}&:&\proof^\ast\\
      \textsf{verification\_keys}&:&\texttt{threshold\_verification\_key}^\ast
    \end{array}
  \right\}
\end{gather*}
...
\item \textsf{signatures} is set to the same value as the
  \textsf{signatures} field of \texttt{vinput}s
...
\begin{gather*}
  ...
  \texttt{trustee\_kind}=
  [\texttt{"Single"},\texttt{basic\_parameters}]\mid
  [\texttt{"Pedersen"},\texttt{threshold\_parameters}]
\end{gather*}

A $\texttt{trustees}$ structure is associated to each election. Such a
structure is a list of either basic parameters as described in
section~\ref{basic-trustees}, or threshold parameters as described in
...
$\texttt{XXXXX-XXXXXX-XXXXX-XXXXXX}$, where the 22 $\texttt{X}$
characters are taken from the Base58 alphabet:
\[\texttt{123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz}\]
From $c$, a secret exponent $x=\secret(c)$ is derived as follows:
\begin{itemize}
\item $(x)\leftarrow\textsf{HashToField}(\texttt{\dstprefix-derive\_credential},\uuid\texttt{|}c,1)$
\end{itemize}
From this secret exponent, a public key $\public(c)=g^x$ is
computed.
\[
  \texttt{public\_credential}=\left\{
    \begin{array}{rcl}
      \textsf{credential}&:&\G\\
      \textsf{?weight}&:&\I
    \end{array}
  \right\}
\]
\[\texttt{public\_credentials}=\texttt{public\_credential}^\ast\]
$\texttt{public\_credentials}$ are published as part of public data.
...
\begin{gather*}
  \texttt{question\_l}=\left\{
    \begin{array}{rcl}
      \answers&:&\jstring^{\ast\ast}\\
      \textsf{question}&:&\jstring
    \end{array}
  \right\}
\end{gather*}
...
There are three types of questions: homomorphic (H) ones,
non-homomorphic (NH) ones and lists (L) ones. A key difference is the
outcome of the election: with H or L questions, only the pointwise sum
of all the answers (see~\ref{answers}) will be revealed at the end of
the election whereas with a NH question, each individual answer will
be revealed.

\subsubsection{Homomorphic questions}

Homomorphic questions were the first type of question that were
implemented in Belenios. They are represented in a $\texttt{question}$
structure, where the $\textsf{type}$ property is set to
\verb=Homomorphic= and the $\textsf{value}$ property is set to a
\texttt{question\_h} structure. They are suitable for many elections,
like the ones where the voter is invited to select one choice among
several (as in a referendum).
...
Non-homomorphic questions are represented in a \texttt{question}
structure, where the \textsf{type} property is set to
\verb=NonHomomorphic=, and the \textsf{value} property is set to a
\texttt{question\_nh} structure. They are needed when homomorphic
questions are not suitable, for example when answers represent
preferences or are too big. An \textsf{extra} field may be present, to
give a hint on the intended counting method (Majority Judgment,
Condorcet, STV, \dots).

\subsubsection{Lists questions}

Lists questions are represented in a \texttt{question} structure,
where the \textsf{type} property is set to \verb=Lists=, and the
\textsf{value} property is set to a \texttt{question\_l}
structure. Such questions are suitable to elect candidates grouped in
lists. Here, $\answers$ is a vector of vectors of strings with the
shape:
\[ [[L_1,C_{1,1},\dots],\dots,[L_n,C_{n,1},\dots]] \] where $L_i$ is
the name of list $i$ and $C_{i,j}$ is the name of candidate $j$ of
list $i$. The voter is asked to select one list $L_i$ among
$L_1,\dots,L_n$ and, in this list, at least one candidate among
$C_{i,1},\dots,C_{i,n_i}$.
...
set to 2 in this version of the specification. It is incremented in
...
hexadecimal encoding of the \textsf{election} field of
...
\begin{gather*}
  \texttt{answer\_l}=\left\{
    \begin{array}{rcl}
      \choices&:&\ciphertext^{\ast\ast}\\
      \iproofs&:&\iproof^{\ast\ast}\\
      \oproof&:&\proof\\
      \textsf{list\_proofs}&:&\proof^{\ast\ast}\\
      \textsf{nonzero\_proof}&:&\texttt{nonzero\_proof}
    \end{array}
  \right\}
\end{gather*}

\begin{gather*}
  \answer=\texttt{answer\_h}\mid\texttt{answer\_nh}\mid\texttt{answer\_l}
\end{gather*}
...
\item $\textsf{proof}$ is computed as follows:
  ...
  \[
    \Hash_{\textsf{raweg}}(S,y,\alpha,\beta,A)=\textsf{HashGroup}(\texttt{\dstprefix-raweg},(S\verb=|=y\verb=,=\alpha\verb=,=\beta\verb=|=),A)
  \]
\end{itemize}
...
\subsubsection{Lists answers}
\label{l-answers}

The plaintext answer to a lists question is a vector of vectors of
bits \[m=[[L_1,C_{1,1},\dots],\dots,[L_n,C_{n,1},\dots]]\] where:
\begin{itemize}
\item $L_i$ represents if list $i$ is selected;
\item $C_{i,j}$ represents if candidate $j$ of list $i$ is selected.
\end{itemize}
The corresponding encrypted answer consists of:
\begin{itemize}
\item $\choices$: the pointwise encryptions of bits of $m$,
  like in homomorphic answers;
\item $\iproofs$: proofs that $m$ contains only bits ($0$ or $1$),
  like in homomorphic answers;
\item $\oproof$: a proof that $\sum_i L_i=1$ (\emph{exactly one list
    is selected});
\item $\textsf{list\_proofs}$: a vector of proofs
  $[\pi_1,\dots,\pi_n]$ where each $\pi_i$ proves that
  $L_i=1\lor\sum_jC_{i,j}=0$ (\emph{each list is either selected, or
    all its candidates are not selected});
\item $\textsf{nonzero\_proof}$: a proof that
  $\sum_i\sum_jC_{i,j}\neq 0$ (\emph{at least one candidate is
    selected}).
\end{itemize}

\paragraph{Encryptions} Each bit $m_{i,j}$ in $m$ is encrypted (in an
El Gamal-like fashion) into a $\ciphertext$ $c_{i,j}$ as follows:
\begin{enumerate}
\item pick a random $r_{i,j}\in\Z_q$
\item $\alphalabel=g^{r_{i,j}}$
\item $\betalabel=y^{r_{i,j}}g^{m_{i,j}}$
\end{enumerate}
\paragraph{Individual proofs} Each individual proof that
$m_{i,j}\in\{0,1\}$ is computed by running:
\[\iprove(S_0,r_{i,j},m,0,1)\]
Like in homomorphic answers, other proofs in lists answers use a
string $S$ built as follows:
\begin{itemize}
\item let $a$ be $\choices$, where each ciphertext $c$ is replaced by
  the serialization of its $\alphalabel$ field, a comma, and the
  serialization of its $\betalabel$ field;
\item let $b$ be the concatenation of all strings in $a$, separated by
  commas;
\item let $S$ be the string $S_0$ followed by a vertical bar and $b$.
\end{itemize}

\paragraph{Overall proof} The overall proof that
$\sum_i L_i=1(=\sum_i m_{i,0})$ is computed by running:
\[\iprove\left(S,\left(\sum_i r_{i,0}\right),\left(\sum_i
      m_{i,0}\right),1,1\right)\]

\paragraph{List proofs} See section~\ref{list-proofs}.

\paragraph{Non-zero proof} To prove that $\sum_i\sum_jC_{i,j}\neq 0$:
\begin{enumerate}
\item compute $r=\sum_i\sum_{j\neq 0}r_{i,j}$,
  $\alpha=\prod_i\prod_{j\neq 0}\alphalabel(c_{i,j})$ and
  $\beta=\prod_i\prod_{j\neq 0}\betalabel(c_{i,j})$;
  \begin{itemize}
  \item $(\alpha,\beta)$ is the encryption of $\sum_i\sum_jC_{i,j}$
    with random $r$;
  \end{itemize}
\item prove that $(\alpha,\beta)$ encrypts a non-zero value with
  random $r$ (see section~\ref{nonzero-proof}).
\end{enumerate}
...
\item $\pi_i$ is created as follows:
  ...
  \end{enumerate}
\end{enumerate}
In the above, $\Hash_\iprove(S,\alpha,\beta,A_0,B_0,\dots,A_k,B_k)$ is computed as follows:
\[\textsf{HashGroup}(\texttt{\dstprefix-prove},(S\verb=|=\alpha\verb=,=\beta\verb=|=),(A_0,B_0,\dotsc,A_k,B_k))\]
We will denote the whole procedure by $\iprove(S,r,i,M_0,\dots,M_k)$.
...
\end{enumerate}
In the above, $\Hash_{\mathsf{bproof0}}(\dotsc)$ is computed as follows:
\[\textsf{HashGroup}(\texttt{\dstprefix-bproof0},S,(A_0,B_0,A_\Sigma,B_\Sigma))\]
...
\end{enumerate}
In the above, $\Hash_{\mathsf{bproof1}}(\dotsc)$ is computed as follows:
\[\textsf{HashGroup}(\texttt{\dstprefix-bproof1},S,(A_0,B_0,\dotsc,A_k,B_k))\]
...
\subsection{Proofs of list constraints}
\label{list-proofs}

\paragraph{Proving}
For each $i$, the proof of the list constraint that
$L_i=1\lor\sum_jC_{i,j}=0$ is a pair $\pi_i=(\pi_{i,0},\pi_{i,1})$
computed as follows:
\begin{itemize}
\item let $r_0=r_{i,0}$, $\alpha_0=\alphalabel(c_{i,0})$ and
  $\beta_0=\betalabel(c_{i,0})$:
  \begin{itemize}
  \item $(\alpha_0,\beta_0)$ is the encryption of $L_i$ with random $r_0$;
  \end{itemize}
\item compute $r=\sum_{j\neq 0}r_{i,j}$,
  $\alpha=\prod_{j\neq 0}\alphalabel(c_{i,j})$ and
  $\beta=\prod_{j\neq 0}\betalabel(c_{i,j})$:
  \begin{itemize}
  \item $(\alpha,\beta)$ is the encryption of $\sum_jC_{i,j}$ with random $r$;
  \end{itemize}
\item if $L_i=1$:
  \begin{enumerate}
  \item pick random $\challenge_1$ and $\response_1$ in $\Z_q$;
  \item compute $A_1=g^{\response_1}\times\alpha^{\challenge_1}$ and
    $B_1=y^{\response_1}\times\beta^{\challenge_1}$;
  \item pick a random $w\in\Z_q$;
  \item compute $A_0=g^w$ and $B_0=y^w$;
  \item compute $h=\Hash_{\textsf{lproof}}(S,A_0,B_0,A_1,B_1)$;
  \item compute $\challenge_0=h-\challenge_1$ and
    $\response_0=w-r_0\times\challenge_0$;
  \end{enumerate}
\item if $\sum_jC_{i,j}=0$:
  \begin{enumerate}
  \item pick random $\challenge_0$ and $\response_0$ in $\Z_q$;
  \item compute $A_0=g^{\response_0}\times\alpha_0^{\challenge_0}$ and
    $B_0=y^{\response_0}\times(\beta_0/g)^{\challenge_0}$;
  \item pick a random $w\in\Z_q$;
  \item compute $A_1=g^w$ and $B_1=y^w$;
  \item compute $h=\Hash_{\textsf{lproof}}(S,A_0,B_0,A_1,B_1)$;
  \item compute $\challenge_1=h-\challenge_0$ and
    $\response_1=w-r\times\challenge_1$;
  \end{enumerate}
\item for $k\in\{0,1\}$, build $\pi_{i,k}$ from $\challenge_k$ and
  $\response_k$.
\end{itemize}
In the above, $\Hash_{\textsf{lproof}}(\dotsc)$ is computed as follows:
\[
  \textsf{HashGroup}(\texttt{\dstprefix-lproof},S,(A_0,B_0,A_1,B_1))
\]

\paragraph{Verifying} A proof composed of $\challenge_k$ and
$\response_k$ can be verified as follows:
\begin{enumerate}
\item compute $\alpha_0$, $\beta_0$, $\alpha$ and $\beta$ as above;
\item compute $A_0=g^{\response_0}\times\alpha_0^{\challenge_0}$ and
  $B_0=y^{\response_0}\times(\beta_0/g)^{\challenge_0}$;
\item compute $A_1=g^{\response_1}\times\alpha^{\challenge_1}$ and
  $B_1=y^{\response_1}\times\beta^{\challenge_1}$;
\item check that
  $\Hash_{\textsf{lproof}}(S,A_0,B_0,A_1,B_1) =
  \challenge_0+\challenge_1$.
\end{enumerate}

\subsection{Proof of encryption of non-zero}
\label{nonzero-proof}

\begin{gather*}
  \texttt{nonzero\_proof}=\left\{
    \begin{array}{rcl}
      \textsf{commitment}&:&\G\\
      \challenge&:&\Z_q\\
      \response&:&\Z_q\times\Z_q
    \end{array}
  \right\}
\end{gather*}

\paragraph{Proving}
To prove that $(\alpha,\beta)$ is the encryption of $m\neq 0$ with
random $r$:
\begin{enumerate}
\item pick a random $s\in\Z_q$ such that $s\neq 0$;
\item compute $A_0=\beta^s\times y^{-s\times r}$:
  \begin{itemize}
  \item notice that $m\neq 0$ is equivalent to $A_0\neq 1$;
  \item we also have $1=\alpha^s\times g^{-s\times r}$;
  \item hence, we resort to proving that $A_0$ (in base $(\beta,y)$)
    and $1$ (in base $(\alpha,g)$) have the same representation;
  \end{itemize}
\item pick random $w_1$ and $w_2$ in $\Z_q$;
\item compute $A_1=\alpha^{w_1}\times g^{w_2}$ and
  $A_2=\beta^{w_1}\times y^{w_2}$;
\item compute $c=\Hash_{\textsf{nonzero}}(S,A_0,A_1,A_2)$;
\item compute $t_1=w_1-s\times c$ and $t_2=w_2+s\times r\times c$;
\item set $\textsf{commitment}$ to $A_0$, $\challenge$ to $c$ and $\response$
  to $(t_1,t_2)$.
\end{enumerate}
In the above, $\Hash_{\textsf{nonzero}}(\dotsc)$ is computed as follows:
\[
  \textsf{HashGroup}(\texttt{BELENIOS-v4-nonzero},S,(A_0,A_1,A_2))
\]

\paragraph{Verifying} Following the same notations as above, a proof
composed of $A_0$, $c$, $t_1$ and $t_2$ can be verified as follows:
\begin{enumerate}
\item check that $A_0\neq 1$;
\item compute $A_1=\alpha^{t_1}\times g^{t_2}$ and
  $A_2=\beta^{t_1}\times y^{t_2}\times A_0^c$;
\item check that $\Hash_{\textsf{nonzero}}(S,A_0,A_1,A_2)=c$.
\end{enumerate}
...
\begin{gather*}
  \texttt{raw\_ballot}=\left\{
    ...
    \end{array}
  \right\}\\
  \ballot=\texttt{raw\_ballot}\ \texttt{signed\_msg}
\end{gather*}
A $\texttt{raw\_ballot}$ references in its $\textsf{credential}$
member the public credential $S=g^{\secret(c,s)}$ ($c$ being the
secret credential) of the voter. The hash (or \emph{fingerprint}) of
the election is $\varphi$ (see section~\ref{varphi}).  A full
$\ballot$ is a $\texttt{raw\_ballot}$ signed with its
$\textsf{credential}$ and domain separation tag
$\texttt{\dstprefix-ballot}$.
...
\begin{gather*}
  \texttt{raw\_shuffle}=\left\{
    \begin{array}{rcl}
      \textsf{ciphertexts}&:&\ciphertext^\ast{}^\ast\\
      \textsf{proofs}&:&\texttt{shuffle\_proof}^\ast
    \end{array}
  \right\}\\
  \texttt{shuffle}=\texttt{raw\_shuffle}\ \texttt{signed\_msg}
\end{gather*}
...
\begin{gather*}
  ...
  \end{array}
\end{gather*}
Each \texttt{shuffle} is signed by a trustee with their PKI key and
domain separation tag \texttt{\dstprefix-shuffle}.  For each
non-homomorphic question $i$:
\begin{enumerate}
...
\begin{gather*}
  \texttt{partial\_decryption\_factor}=\left\{
    \begin{array}{rcl}
      \textsf{factor}&:&\G\\
      \textsf{proof}&:&\proof
    \end{array}
  \right\}\\
  \texttt{raw\_partial\_decryption}=\texttt{partial\_decryption\_factor}^\ast{}^\ast\\
  \pdecryption=\texttt{raw\_partial\_decryption}\ \texttt{signed\_msg}
\end{gather*}
...
so-called \emph{raw decryption factors} $F_{i,j}$ such that:
\[
\textsf{factor}(F_{i,j})=\alphalabel(a'_{i,j})^x
\]
and proofs that they were correctly computed. Each
$\textsf{proof}(F_{i,j})$ is computed as follows:
\begin{enumerate}
\item pick a random $w\in\Z_q$
...
\[
\Hash_\decrypt(X,A,B)=\textsf{HashGroup}(\texttt{\dstprefix-decrypt},(\varphi\verb=|=X\verb=|=),(A,B))
\]
...
A full \texttt{partial\_decryption} is a
\texttt{raw\_partial\_decryption} signed with the PKI key of its
owner, with the domain separation tag
\texttt{\dstprefix-partial\_decryption}.
...
A group is identified by a short string. In addition to the usual
...
\item $g$ is the unique point in $E(\F_p)$ whose $y$ coordinate is
  $4/5$ and whose $x$ coordinate is positive,
...
\subsection{Hashing to base field elements}
\label{hash-to-field}

The function $\textsf{HashToField}(d,m,n)$ is used to hash an
arbitrary message $m$ into $n$ elements of the scalar field $\Z_q$. It
is based on \verb|hash_to_field| from RFC 9380, using
\verb|expand_message_xmd| with \verb|SHA256|. $d$ is the domain
separation tag.

\subsection{Hashing group elements}
\label{hash-group}

The function $\textsf{HashGroup}(d,m,(y_1,\dotsc,y_n))$ is used to
hash an arbitrary message $m$ and $n$ group elements $y_1,\dotsc,y_n$
into a single field element $x$ as follows:
\begin{itemize}
\item let $G$ be the group name
\item let $Y$ be the string serializations of $y_1,\dots,y_n$
  concatenated with commas
\item
  $(x)\leftarrow\textsf{HashToField}(d\texttt{-group\_hash-}G,mY,1)$
\end{itemize}
...
\begin{table}
  \begin{framed}
    ...
    \begin{enumerate}
    \item
      $\textbf{u}\leftarrow\textsf{HashToField}(\texttt{\dstprefix-get\_nizkp\_challenges},S,N)$
    \item Return $\textbf{u}$\hfill// $\textbf{u}\in\Z_q^N$
    \end{enumerate}
  \end{framed}
  \caption{Function $\mathsf{GetNIZKPChallenges}(N,S)$}
...
\begin{table}
  \begin{framed}
    ...
    \begin{enumerate}
    \item
      $(c)\leftarrow\textsf{HashToField}(\texttt{\dstprefix-get\_nizkp\_challenge},S,1)$
    \item Return $c$\hfill// $c\in\Z_q$
    \end{enumerate}
  \end{framed}
  \caption{Function $\mathsf{GetNIZKPChallenge}(S)$}
