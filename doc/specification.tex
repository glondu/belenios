...
items, one or several trustees run the matching protocol below to
produce a sub-key $y_\gamma$. Once all protocols have been run,
$\mathcal{A}$ synthesizes the global election public key $y$ from the
sub-keys computed in each protocol by multiplying them:
\[
  y=\prod_\gamma y_\gamma
\]
...
\item $\mathcal{T}$ generates a \hyperref[trustee-keys]{$\tpk$}
  $k$ and sends it to $\mathcal{A}$
\item $\mathcal{A}$ checks $k$
\end{enumerate}
Later, when the election is open:
\begin{enumerate}
\item $\mathcal{T}$ checks that $k$ appears in the set of public
...
$\mathcal{T}_1,\dots,\mathcal{T}_\mu$ such that a only a subset of
...
\item for $z\in[1\dots \mu]$, $\mathcal{T}_z$ checks that $\gamma_z$
  appears in the set of public keys $PK$ of the election of {$\uuid$}
  $u$ (the id of the election should be publicly known).
\end{enumerate}
The sub-key for this protocol is computed as specified in
section~\ref{polynomials}.
...
the $\texttt{trustees}$ structure. For each of its items, a sub-factor
$F_{i,j,\gamma}$ is computed:
\begin{itemize}
\item for a \texttt{"Single"} item corresponding to trustee
  $\mathcal{T}_z$:
  \[
    F_{i,j,\gamma}=\pdlabel_{z,i,j}
  \]
\item for a \texttt{"Pedersen"} item corresponding to trustees
  $\mathcal{T}_{z_1},\dots,\mathcal{T}_{z_\mu}$:
  \[
    F_{i,j,\gamma}=\prod_{\delta\in\mathcal{I}}(\pdlabel_{z_\delta,i,j})^{\lambda_\delta^{\mathcal{I}}}
  \]
  where $\mathcal{I}$ is the set of ($t+1$) indexes of supplied
  partial decryptions, relative to
  $\mathcal{T}_{z_1},\dots,\mathcal{T}_{z_\mu}$, and
...
\[
  F_{i,j}=\prod_\gamma F_{i,j,\gamma}
\]
